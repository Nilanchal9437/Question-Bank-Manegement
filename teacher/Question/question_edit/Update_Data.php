        ...
        if(move_uploaded_file($file_tmp_name, $file_full_path))
        {
            ...
            if(isset($_SESSION['user_id'])){
                ...
                if($result->num_rows > 0){
                    $row = $result->fetch_assoc();
                    $id = $row['t_id'];
                    $sql = "UPDATE university_questions SET uni_name='$uni_name',course = '$course', branch = '$branch',subject = '$subject',semester = '$semster',year = '$year', file_location = '$file_full_path', uploaded_by = '$id', approved = 0,date_uploaded = '$upload_date' WHERE q_id = $q_id";
